...
- [X] T137b Restrict the certificate authority's private key on Windows, in `internal/trust/restrict_windows.go` with a no-op sibling for POSIX. **Found by CI, not by design.** Windows does not implement POSIX permission bits: the key was written 0600 and reported 0666, because access there is decided by a list this code never set. It inherited the list on `%LOCALAPPDATA%`, which is user-only on a default installation — the operating system's arrangement rather than a guarantee fastr made, on the most sensitive artefact in the product. The list is now written explicitly and, crucially, **protected**: without `PROTECTED_DACL_SECURITY_INFORMATION` the inherited entries are merged back in and the result grants more than it names. **Two keys, not one.** The task said the authority's key; `issue.go` writes `server.key` with the same 0600 and the same non-effect, so both are covered and so is the directory, whose entry is inheritable so anything written there later starts out restricted. `LoadOrCreate` repairs an authority left behind by an older build, because that is the call trusted-mode setup makes and the moment the user is told the key stays on their machine; `Load` remains a read and does not. The Windows tests read the list back as SDDL rather than walking the ACL, whose entry count `x/sys/windows` does not export, and the repair test first widens the list and asserts it widened — a repair test that starts from an already-narrow list proves nothing. What this does **not** do is put the key beyond an administrator, and nothing on Windows can: `SeTakeOwnershipPrivilege` rewrites any list on the machine. It removes every *other* account that happened to be named on the parent.
...
- [X] T137 Decide the pairing hardening question, implementing a PAKE or documenting the accepted risk, in `internal/pairing/handshake.go`, per research item 1. **Implemented CPace**, the CFRG's balanced PAKE, in the CPACE-RISTRETTO255-SHA512 suite of draft-irtf-cfrg-cpace-14. Writing the tests found something worse than the flagged risk: the joining device **sent the six digits in the body of the confirm request**, so on a plain-HTTP network an observer read the code rather than having to search for it. The flagged offline oracle was real too — anyone who could play the host's part once kept a tag testable against all 10^6 candidates. CPace removes both: the code is mapped to a group generator on each side and never transmitted, so the whole transcript is independent of it and an attacker has to commit to one candidate before the exchange starts. One interaction is one guess, and there are five. Chosen over SPAKE2 because the draft **publishes a complete test vector for exactly this suite**, and two implementations that must agree byte for byte need a third party's answer rather than each other's: appendix B.3 is checked in `internal/pairing/cpace_test.go` and in `web/scripts/verify-crypto.ts`. The secrecy property is asserted against recorded traffic in `test/integration/pairing_secrecy_test.go` and, against the client that actually ships, in `web/tests/e2e/pairing_secrecy.spec.ts`; both were verified to fail by putting the code back in the request. Protocol version 2; existing pairings are invalidated, which is one screen. Bundle cost: about 8 kB compressed.
...
- [X] T140 [P] Run the full accessibility gate and fix every violation, in `web/tests/e2e/a11y.spec.ts` rather than the path this task named, alongside the other browser specs.
- [X] T141 [P] Verify progress announcements fire only at meaningful moments in `web/tests/e2e/a11y_announcements.spec.ts`, per FR-039i. Same relocation as T140.
...
- [X] T145 Measure idle background resource use against the budget in `test/integration/idle_budget_test.go`, per SC-018. It measures the **real binary**, because the timers the budget is about — the mDNS listener, the sweeper, the acceptance ticker, the reachability prober — exist only in the assembled program. Read from the kernel's own accounting rather than sampled, so a burst between two samples cannot hide. Measured at **0 ms of processor time across twelve idle seconds and 11 MB resident**, against budgets of 1% and 100 MB. The reader is proved against a deliberate busy loop, because the obvious self-check — "startup must have used something" — turned out to be false: fastr's entire startup costs less than the kernel's 10 ms tick. **Measured on both systems now.** The Windows half is a different implementation rather than a tweak, as expected: `GetProcessTimes` and `K32GetProcessMemoryInfo` against a handle, the latter declared locally because `x/sys/windows` exports only the working-set *limits* and not the usage. Windows reads **0 ms and 18 MB**, cross-checked against `Get-Process` to the megabyte. **The test was also passing on a process that was not running**: the binary refuses to start without the web bundle, and a dead process reports no processor time and an empty working set, which is a perfect score. It now keeps the instance's output, asserts it is still alive at both ends of the window, and refuses a memory reading under a megabyte.
- [X] T146 [P] Verify every error in the catalogue names a cause and a corrective action in `test/integration/error_catalogue_test.go`, per SC-014. Both directions: every code used in the source is in the catalogue (found by parsing the Go, so a code added in a hurry with the message left for later is caught), every message says what happened *and* what to do, and none leaks a code, a path, or an HTTP status. **The check had to be sharpened twice.** Matching an imperative anywhere in the message passed "It may have finished or been cleared" on "finish" and "It will start on its own" on "start" — instructions to nobody. Matching at the *start of a sentence*, which is where an imperative lives in both languages, made the two languages agree on exactly the same failures, which is the sign the rule is now precise. Six messages legitimately give no instruction and are listed with the reason each is right not to; `invalid_path` was not one of them and now tells the user to rename the file.
- [X] T147 [P] Write the user documentation covering both protection modes, in `docs/using-fastr.md`. It **leads** with what is and is not protected, because a page that explains that at the bottom is a page most readers never reach it in. It also ends with a "known limits" section naming the Windows key permissions, the unverified setup steps, and the missing streamed decryption — stated there rather than discovered later.
- [ ] T148 Update `README.md` with real install and run instructions and remove the pre-alpha banner. **Half done, and the other half is deliberately not.** The status and the roadmap were factually wrong — they said there was no working build while all six stories worked — and now say what exists, what blocks a release, and how to build it. The banner itself stays until T137 is decided and T149 has been performed: removing it now would claim a readiness the project does not have, which is the same dishonesty Principle V forbids about protection modes.
...
- [X] T151 Give the interface a hierarchy, in `web/src/lib/Panel.svelte` and `web/src/routes/App.svelte`. **Nothing was wrong with any single panel; the problem was that there were nine of them and every one was drawn identically.** Each carried its own copy of the same card and the same heading size, so the send zone — the thing Principle VI puts a three-action ceiling on — sat in the middle of a stack with the same weight as last week's history. One shared panel component now owns that decision: `tone` (hero, plain, quiet, urgent) states the hierarchy in one place, and anything that is not the current task folds behind its own title. Folding is never applied to what the constitution requires on screen: the protection notice does not fold, a device waiting for approval does not fold, and a running transfer opens itself. The two audits that read the whole screen — accessibility and translation coverage — now unfold every panel first, because a presentation decision must not quietly shrink what they check.
